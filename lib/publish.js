...
function publish (uri, data, credentials, tarball, cb) {
  if (!(credentials.token ||
        (credentials.password && credentials.username && credentials.email))) {
  ...
  fs.stat(tarball, function(er, s) {
    ...
    fs.readFile(tarball, function(er, tarbuffer) {
      if (er) return cb(er)
      putFirst.call(self, uri, data, tarbuffer, s, credentials, cb)
    })
  })
}
...
function putFirst (registry, data, tarbuffer, stat, creds, cb) {
  ...
  this.request("PUT", fixed, { body : root, auth : creds }, function (er, parsed, json, res) {
    ...
    this.request("GET", fixed + "?write=true", { auth : creds }, function (er, current) {
      if (er) return cb(er)

      putNext.call(this, registry, data.version, root, current, creds, cb)
    }.bind(this))
  }.bind(this))
}

function putNext (registry, newVersion, root, current, creds, cb) {
  ...
  this.request("PUT", uri, { body : current, auth : creds }, cb)
}
