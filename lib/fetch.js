...
function fetch (uri, headers, credentials, cb) {
  ...
  this.attempt(function (operation) {
    makeRequest.call(client, uri, headers, credentials, function (er, req) {
      if (er) return cb(er)
...
function makeRequest (remote, headers, credentials, cb) {
  var parsed = url.parse(remote)
  this.log.http("fetch", "GET", parsed.href)

  var er = this.authify(
    credentials.alwaysAuth,
    parsed,
    headers,
    credentials
  )
