...
function regRequest (method, uri, options, cb_) {
  ...
  this.log.verbose("request", "on initialization, where is", uri)
  ...
  this.attempt(function (operation) {
    makeRequest.call(self, method, uri, what, options.etag, follow, authed, auth
...
function makeRequest (method, where, what, etag, follow, authed, credentials, cb_) {
  var cb = once(cb_)
  ...
  var er = this.authify(authed, parsed, headers, credentials)
  if (er) return cb_(er)
