...
function get (uri, params, cb) {
  ...
  if (uri.match(/\?write=true$/)) {
    return get_.call(this, uri, cache, params, cb)
  }
  ...
  fs.stat(cache, function (er, stat) {
    if (!er) fs.readFile(cache, function (er, data) {
      ...
      params.stat = stat
      params.data = data
      get_.call(this, uri, cache, params, cb)
    }.bind(this))
    else {
      get_.call(this, uri, cache, params, cb)
    }
...
function requestAll_ (uri, c, data, cache, cb) {
  ...
  this.request(uri, {}, function (er, updates, _, res) {
...
function get_ (uri, cache, params, cb) {
  ...
  this.request(uri, { etag : etag, follow : follow, auth : auth }, function (er, remoteData, raw, response) {
