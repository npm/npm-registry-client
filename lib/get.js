...
function requestAll (cb) {
  ...
  var lockFileName = cache + ".lock"

  mkdir(path.join(this.conf.get('cache'), "-", "all"), function (er) {
    
    lock.call(this, lockFileName, function (er) {
      if (er) return requestAll_.call(this, 0, {}, cb)
      
      fs.readFile(cache, function (er, data) {
        if (er) return unlockThenRequestAll_.call(this, lockFileName, 0, {}, cb)
        try {
          data = JSON.parse(data)
        } catch (ex) {
          return fs.writeFile(cache, "{}", function (er) {
            if (er) return cb(new Error("Broken cache."))
            return unlockThenRequestAll_.call(this, lockFileName, 0, {}, cb)
          }.bind(this))
        }
        var t = +data._updated || 0
        unlockThenRequestAll_.call(this, lockFileName, t, data, cb)
      }.bind(this))
...
function unlockThenRequestAll_ (lockFileName, c, data, cb) {
  lockFile.unlock(lockFileName, function () {
    requestAll_.call(this, c, data, cb)
  }.bind(this))
}
...
function requestAll_ (c, data, cb) {
  ...
  this.request('GET', uri, function (er, updates, _, res) {
    ...
    lock.call(this, lockFileName, function (er) {
      ...
      // Failed to get a lock to update
      if (er) {
        this.log.warn("Failed to get a lock to update the local index")
      ...
      fs.writeFile( cache, JSON.stringify(data)
                  , function (er) {
        if (er) {
          this.log.warn("Failed to write to local index")
        }
        
        lockFile.unlock(lockFileName, function (er) {
          delete data._updated
          return cb(er, data)
        })
      }.bind(this))
...
function lock(filename, cb) {
  var opts = { stale: this.conf.get("cache-lock-stale")
             , retries: this.conf.get("cache-lock-retries")
             , wait: this.conf.get("cache-lock-wait") }
  lockFile.lock(filename, opts, cb)
}
